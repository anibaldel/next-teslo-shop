...
const getPtoducts =async(req: NextApiRequest, res: NextApiResponse<Data>)=> {
    ...
    try {
        ...
        product.images.forEach( async(image)=> {
            if( !images.includes(image)) {
                ...
                await cloudinary.uploader.destroy( fieldId);
            }
        });
...
const createProduct = async(req: NextApiRequest, res: NextApiResponse<Data>) => {
    ...
    if ( images.length < 2 ) {
        return res.status(400).json({ message: 'Es producto necesita almenos 2 imágenes' });
    }
